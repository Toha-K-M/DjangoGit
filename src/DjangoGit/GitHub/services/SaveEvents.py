...
from ..models import GitRepo, GitEvents, Pusher, Sender, PullRequest
...
class SaveEvents(Service):
    ...
    def process(self):
        try:
            ...
            repo = GitRepo.objects.filter(id_repo = data["repository"]["id"])
            if repo.exists():
                gitEvents.repository_id = repo.first().id
